...
const RecitePlayer = () => {
  ...
  const [{ allPodcasts, audio, isAudioPlaying, miniPlayer }, dispatch] = useStatevalue();
  ...
  const nextTrack = () => {
    if (audio > allPodcasts?.Podcasts.length) {
      dispatch({
  ...
  return (
    <div className="w-full full flex items-center gap-3 overflow-hidden">
      ...
      >
        <img
          src={allPodcasts?.Podcasts[audio]?.imageURL}
        ...
        <div className="flex items-start flex-col">
          <p className="text-xl text-headingColor font-semibold">
            {`${
              allPodcasts?.Podcasts[audio]?.name.length > 20
                ? allPodcasts?.Podcasts[audio]?.name.slice(0, 20)
                : allPodcasts?.Podcasts[audio]?.name
            }`}{" "}
            <span className="text-base">| {allPodcasts?.Podcasts[audio]?.Reciter}</span>
          </p>
          <p className="text-textColor">
            {allPodcasts?.Podcasts[audio]?.Album}{" "}
          </p>
          <motion.i
            whileTap={{ scale: 0.8 }}
        ...
        <div className="flex-1">
          <AudioPlayer
            src={allPodcasts?.Podcasts[audio]?.PodcastURL}
...
export const PlayListCard = () => {
  const [{ allPodcasts, audio, isAudioPlaying }, dispatch] = useStatevalue();
  useEffect(() => {
    // console.log(audio);
    if (!allPodcasts) {
  ...
  return (
    <div className="absolute left-4 bottom-24 gap-2 py-2 w-350 max-w-[350px] h-510 max-h-[510px] flex flex-col overflow-y-scroll scrollbar-thin rounded-md shadow-md bg-primary">
      { 
      allPodcasts?.Podcasts.length > 0 ? (
        allPodcasts?.Podcasts.map((Audio, audioindex) => (
          <motion.div
            key={audioindex}
